...
class MDNRNN(object):
    ...
    def build_model(self, hps):
        ...
        use_input_dropout = False if self.hps.use_intput_dropout == 0 else True
        ...
        if use_recurrent_dropout:
            cell = cell_fn(hps.rnn_size, layer_norm=use_layer_norm, dropout_keep_prob=self.recurrent_bropout_prob)
        else:
        ...
        output = tf.reshape(output, [-1, hps.KMIX * 3])
        ...
        def get_lossfunc(logmix, mean, logstd, y):
            v = tf_lognoemal(y, mean, logstd)
            v = tf.reduce_logsumexp(v, 1, keepdims=True)
            return - tf.reduce_mean(v)
        flat_target_data = tf.reshape(self.output_x,[-1, 1]
        lossfunc = get_lossfunc(out_logmix, out_mean, flat_target_data)
