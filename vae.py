...
# Building the CVAE Model within a class

class ConvVAE(object):
    
    #Initializing all the parameters and the variables of the ConvVAE Class
    def __init__(self, z_size=32, batch_size=1, learning_rate=0.0001, kl_tolerance=0.5, is_training=False, reuse=False, gpu_mode=False):
        ...
        self.k1_tolerance = kl_tolerance
       ...
       h = tf.layers.conv2d(self.x, 64, 4, strides=2, activation=tf.nn.relu, name="enc_conv2")
       ...
       h = tf.layers.conv2d_transpose(h, 36, 6, strides=2, activation=tf.nn.relu, name='dec_deconv3')
       ...
       if self.is_training:
           ...
           self.train_op = self.optimizer.apply_gradients(self.loss)
